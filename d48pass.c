...
void pass1(void)
{
	...
	pc = mbank = 0;
	mask = (int) (PF_CLREF | PF_SPLIT | PF_MB0 | PF_MB1 | PF_FORCE);
	...
	for (i=0; i<PMEMSIZE; )
	{
		...
		if (pgmflags[i] == PF_INIT || i > PMEMSIZE - 1)
...
void pass2(void)
{
	...
	fprintf(fp, "\n;  %02d/%02d/%02d %d:%02d", date_time->tm_mon + 1,
...
//
	for (i=0; i<PMEMSIZE; )
	{
		...
		if (pgmflags[i] == PF_INIT)	// ignore un-initialized data
		{
...
//
			...
			(!(pgmflags[i] & PF_REF)) && (!pgmmem[i] || pgmmem[i] == 0xff) &&
			(!(pgmflags[i] & PF_CLREF)) && !(pgmflags[i] & PF_NOINIT) && !(pgmflags[i] & PF_FORCE))
		{
			pgmflags[i] = PF_INIT;	// flag as uninitialized data
			i++;
			skip = TRUE;
